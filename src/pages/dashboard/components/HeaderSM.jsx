...
export default function HeaderSM({user}) {
  ...
  return (
    <div className='w-full p-3 flex flex-col relative'>
        <div className='w-full text-center py-5 px-3 text-2xl flex justify-between items-center'>
            <img src="https://zynex.info/images/logo-home-dark.png" className='w-1/2' alt="ZYNEX logo" />
            <div className={nav ? 'hidden' : 'cursor-pointer'} onClick={() => setNav(true)}>
                <TbMenu2 />
            </div>
            <div className={nav ? 'cursor-pointer' : "hidden"} onClick={() => setNav(false)}>
                <IoClose/>
            </div>
        </div>
        ...
        <div className={`w-full flex flex-col gap-3 pt-5 px-5 z-50 bg-slate-50  absolute top-full left-0 shadow-2xl ${nav ? 'pb-10 h-fit opacity-100' : 'h-0 opacity-0'}  overflow-hidden duration-300`}>
            ...
            <Link href="/dashboard/Events/" className={pathname.includes('Events') ? active : deactive}>
